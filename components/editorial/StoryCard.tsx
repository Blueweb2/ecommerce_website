...
export default function StoryCard({ story, priority = false }: { story: Story; priority?: boolean }) {
  ...
  return (
    <article className="group animate-[fadeIn_600ms_ease-out_both]">
      ...
      <div className="pt-5">
        ...
        <Link href={`/editorial/${story.slug ?? story._id}`} className="mt-2 inline bg-[linear-gradient(currentColor,currentColor)] bg-[length:0_1px] bg-left-bottom bg-no-repeat font-brand-serif text-2xl leading-tight text-stone-900 transition-[background-size] duration-300 group-hover:bg-[length:100%_1px]">{story.title}</Link>
