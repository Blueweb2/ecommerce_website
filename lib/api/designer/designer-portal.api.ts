...
export const getMyCoupons = async (): Promise<any[]> => {
  const response = await api.get("/designers/coupons");
  return extractList(response.data);
};

export const createMyCoupon = async (payload: any): Promise<any> => {
  const response = await api.post("/designers/coupons", payload);
  const data = response.data;
  if (data?.data) return data.data;
  return data;
};

export const updateMyCoupon = async (id: string, payload: any): Promise<any> => {
  const response = await api.put(`/designers/coupons/${id}`, payload);
  const data = response.data;
  if (data?.data) return data.data;
  return data;
};

export const deleteMyCoupon = async (id: string): Promise<void> => {
  await api.delete(`/designers/coupons/${id}`);
};
